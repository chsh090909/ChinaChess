...
class GameFunctions(object):
    """
    GameFunctions中定义了所有的方法
    """
    __logger = logging.getLogger('main.game_fn')
    ...
    def catch_exception(func):
        def wrapper(*args, **kwargs):
            try:
                ...
                # 两种方式任选其一
                GameFunctions.__logger.exception('!!!!!!!!!Error!!!!!!!!!!')
    ...
    def getRandomAllPieces(self):
        ...
        GameFunctions.__logger.info('allPieces: %s' % allPieces)
        return allPieces
    ...
    def getPiecesStatus(self, bool):
        ...
        GameFunctions.__logger.info('revealedBoxes: %s' % revealedBoxes)
    ...
    def hasWon(self, displaySurf, revealedBoxes, all_pieces, playerinfo):
        ...
        GameFunctions.__logger.info('hasWon==>noneCount: %d' % noneCount)
        GameFunctions.__logger.info('hasWon==>redCount: %d' % redCount)
        GameFunctions.__logger.info('hasWon==>blackCount: %d' % blackCount)
    ...
    def getfilename(self, filename):
        ...
        filename = filename1 + '.' + filenamelist[1]
        return filename
